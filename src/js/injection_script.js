...
ensureDomLoaded(() => {
    ...
    domainBypass("imgkoc.buzz", () => {
        ifElement("#newImgE", img => {
            wuLu()
    ...
    domainBypass(/ay\.link|(shtms|aylink)\.co/, () => {
        ...
        awaitElement("#go-link input[name=csrf]", function () {
            const regex = /_a = '(.+)'/
            const regex1 = /_t = '(.+)'/
            const regex2 = /_d = '(.+)'/
            let matches;
            ...
            let matches2;
            document.querySelectorAll("script").forEach(script => {
                matches = regex.exec(script.textContent)
                matches1 = regex1.exec(script.textContent)
                matches2 = regex2.exec(script.textContent)
                var csrf = $("#go-link input[name=csrf]").val();
                var alias = $("#go-link input[name=alias]").val();
                if (matches && matches[1] && matches1 && matches1[1] && matches2 && matches2[1] && matches[1] !== '(.+)') {
                    var data = '_a=' + matches[1] + '&_t=' + matches1[1] + '&_d=' + matches2[1];
                    $.post('https://aylink.co/get/tk', data, function (data1) {
                        $.post('https://aylink.co/links/go2', 'alias=' + alias + '&csrf=' + csrf + '&tkn=' + data1.th, function (data) {
